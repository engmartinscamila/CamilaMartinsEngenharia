...
begin
  ...
  select
    ...
    cr.payment_terms,cr.total_value
  into v_commercial
  ...
  if nullif(btrim(coalesce(v_commercial.experience_level,'')),'') is not null then
    select jsonb_build_object(
      ...
      'contractClauses',to_jsonb(l.contract_clause_refs),
      'catalogVersion',l.version
    )
    into v_level
  ...
  select coalesce(jsonb_agg(
    (
      coalesce(
        ...
        jsonb_build_object(
          'code',s.service_code,
          'name',coalesce(sc.name,s.service_name),
          'description',sc.description,
          'deliverables',coalesce(sc.deliverables,'[]'::jsonb),
          ...
          'catalogVersion',sc.version,
          'levelApplicable',coalesce(sc.level_applicable,false),
          'level',case
            when coalesce(sc.level_applicable,false) and v_level is not null then v_level
            else null
          end
        )
      )
      || jsonb_build_object(
        'included',s.included,
        'acceptanceRequired',s.acceptance_required,
  ...
  v_data := jsonb_build_object(
    ...
    'property_address',concat_ws(', ',
      nullif(v_project.endereco_obra,''),
      nullif(v_project.numero_obra,''),
      nullif(v_project.complemento_obra,''),
      nullif(v_project.bairro_obra,''),
      nullif(v_project.cidade_obra,''),
      nullif(v_project.estado_obra,'')
    ),
  ...
  values(
    auth.uid(),'prepare_contract_document','documentos',v_document_id,
    jsonb_build_object(
