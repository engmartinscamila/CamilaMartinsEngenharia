...
begin
  if auth.role() <> 'service_role' and not public.is_portal_admin() then raise exception 'Acesso administrativo necessário'; end if;
  select coalesce(jsonb_object_agg(t.code,jsonb_build_object('body',t.body,'title',t.title,'version',t.version,'documentKind',t.document_kind,'contractClauses',to_jsonb(t.contract_clause_refs),'contractMasterVersion',t.last_contract_master_version)),'{}'::jsonb)
  into v_result from public.document_text_catalog t where t.active=true and t.document_kind in (p_document_kind,'geral');
  return v_result;
end
$function$;
...
begin
  if auth.role() <> 'service_role' and not public.is_portal_admin() then raise exception 'Acesso administrativo necessário'; end if;
  select coalesce(jsonb_object_agg(t.code,jsonb_build_object('body',t.body,'title',t.title,'version',t.version,'documentKind',t.document_kind,'contractClauses',to_jsonb(t.contract_clause_refs),'contractMasterVersion',t.last_contract_master_version)),'{}'::jsonb)
  into v_result from public.document_text_catalog t where t.active=true;
  return v_result;
end
$function$;

create or replace function public.enqueue_document_text_reviews(p_contract_master_version integer,p_reason text,p_document_kinds text[] default null::text[])
returns integer
language plpgsql
security definer
set search_path = public
...
begin
  if auth.role() <> 'service_role' and not public.is_portal_admin() then raise exception 'Acesso administrativo necessário'; end if;
  insert into public.document_rule_reviews(contract_master_version,source_type,source_code,clause_refs,reason,status,created_at,resolved_at,resolved_by)
  ...
  where t.active=true and (p_document_kinds is null or t.document_kind='geral' or t.document_kind=any(p_document_kinds))
  on conflict(contract_master_version,source_type,source_code) do update set clause_refs=excluded.clause_refs,reason=excluded.reason,status='pending',created_at=now(),resolved_at=null,resolved_by=null;
...
begin
  if auth.role() <> 'service_role' and not public.is_portal_admin() then raise exception 'Acesso administrativo necessário'; end if;
  if v_level_code is not null then select * into v_level from public.service_level_catalog where code=v_level_code and active=true; end if;
  for v_item in select * from jsonb_array_elements(coalesce(p_services,'[]'::jsonb)) loop
    ...
    if found then
      ...
      if v_catalog.level_applicable and v_level.code is not null then
        v_level_json := jsonb_build_object('code',v_level.code,'label',v_level.label,'subtitle',v_level.subtitle,'description',v_level.description,'features',v_level.features,'exclusions',v_level.exclusions,'contractClauses',to_jsonb(v_level.contract_clause_refs),'catalogVersion',v_level.version);
      end if;
      v_result := v_result || jsonb_build_array(jsonb_build_object('code',v_catalog.code,'name',v_catalog.name,'included',coalesce((v_item->>'included')::boolean,false),'value',v_item->'value','notes',nullif(btrim(v_item->>'notes'),''),'acceptanceRequired',v_catalog.acceptance_required,'displayOrder',coalesce((v_item->>'displayOrder')::integer,ascii(left(v_catalog.code,1))-96),'description',v_catalog.description,'deliverables',v_catalog.deliverables,'exclusions',v_catalog.exclusions,'clientInputs',v_catalog.client_inputs,'revisions',v_catalog.default_revisions,'deliveryFormats',v_catalog.delivery_formats,'planningReference',v_catalog.planning_reference,'contractClauses',to_jsonb(v_catalog.contract_clause_refs),'catalogVersion',v_catalog.version,'levelApplicable',v_catalog.level_applicable,'level',v_level_json));
    else
