...
(function moduloCronograma() {
    ...
    function renderizar(lista = etapas, abrirPastas = false) {
        ...
        if (!lista.length) {
            corpo.innerHTML = '<tr><td class="estado-vazio">Nenhuma etapa cadastrada.</td></tr>';
            return;
        }

        const porCliente = lista.reduce((mapa, etapa) => {
            const chave = String(etapa.cliente_id || "sem-cliente");
            (mapa[chave] ||= []).push(etapa);
            return mapa;
        }, {});

        corpo.innerHTML = Object.values(porCliente).map(grupoCliente => {
            const primeira = grupoCliente[0];
            const ordenadas = [...grupoCliente].sort((a, b) =>
                Number(a.ordem || 0) - Number(b.ordem || 0)
            );

            return `
                <tr class="cme-folder-row">
                    <td>
                        <details class="pasta-cliente cme-pasta-cliente" ${abrirPastas ? "open" : ""}>
                            <summary>
                                ...
                                <span class="cme-pasta-contagem">${grupoCliente.length} ${grupoCliente.length === 1 ? "etapa" : "etapas"}</span>
                            </summary>
                            <div class="cme-pasta-corpo">
                                <table class="cme-cronograma-tabela">
                                    <thead>
                                        <tr>
                                            <th>Etapa</th>
                                            <th>Projeto</th>
