...
export function buildLicensedTo(params: { isPublicSample: boolean; fullName?: string | null; email?: string | null; }): string {
  if (params.isPublicSample) return "AMOSTRA PUBLICA - CAMILA MARTINS ENGENHARIA";
...
export function sanitizeFilename(value: string): string {
  const slug = normalizeText(value, 100).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
...
export async function createIssueCode(secret: string, seed: string): Promise<string> {
...
export function validPortalPath(value: unknown): value is string {
  ...
  return path.length > 4 && path.length <= 700 && !path.startsWith("/") && !path.includes("..") && !path.includes("\\") && path.toLowerCase().endsWith(".pdf");
}
