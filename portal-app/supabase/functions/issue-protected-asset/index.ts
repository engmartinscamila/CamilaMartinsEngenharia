...
async function stampPdf(original: Uint8Array, identity: string, contractNumber: string, code: string) {
  ...
  for (const page of document.getPages()) {
    ...
    const watermark = `${identity} • ${contractNumber} • ${code}`;
    page.drawText(watermark, {
      x: Math.max(24, width * 0.12),
      y: height * 0.48,
      size: Math.max(13, Math.min(24, width / 28)),
      font,
      color: rgb(0.45, 0.37, 0.23),
      opacity: 0.17,
      rotate: degrees(32),
    });
...
<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="1200" viewBox="0 0 1600 1200">
  <rect width="1600" height="1200" fill="#010914"/>
  <image href="${source}" width="1600" height="1200" preserveAspectRatio="xMidYMid meet"/>
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (shouldTransform) {
      ...
      if (kind === 'document') {
        if (original.type !== 'application/pdf' && !asset.arquivo.toLowerCase().endsWith('.pdf')) {
          return json({ error: 'Documento autoral deve ser enviado em PDF.' }, 400);
        }
        issuedPath = `issued/${userData.user.id}/document-${asset.id}-${code}.pdf`;
        const stamped = await stampPdf(originalBytes, identity, contractNumber, code);
        const upload = await service.storage.from(issuedBucket).upload(issuedPath, stamped, { contentType: 'application/pdf', upsert: false });
        if (upload.error) throw upload.error;
      } else {
        issuedPath = `issued/${userData.user.id}/photo-${asset.id}-${code}.svg`;
        const stamped = watermarkedSvg(originalBytes, original.type || 'image/jpeg', identity, contractNumber, code);
        const upload = await service.storage.from(issuedBucket).upload(issuedPath, stamped, { contentType: 'image/svg+xml', upsert: false });
        if (upload.error) throw upload.error;
      }
    ...
    const signed = await service.storage.from(issuedBucket).createSignedUrl(issuedPath, 600, {
      download: action === 'download' ? asset.nome ?? true : false,
    });
    ...
    return json({
      ...
      expiresAt,
      trackingCode: code,
      protectedCopy: shouldTransform,
      downloadAllowed: action === 'download',
      notice: shouldTransform ? 'Cópia identificada. Capturas de tela não podem ser impedidas completamente.' : null,
    });
  } catch (error) {
