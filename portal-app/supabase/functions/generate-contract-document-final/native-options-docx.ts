...
function build(kind:string,d:Obj,profile:Profile){
 ...
 if(kind==='termo_aceite')return makeDoc([
  ...title('TERMO DE ACEITE DE ETAPA','Validação de entrega prevista no fluxo contratual'),...identity(d,profile),h('2. ETAPA ENTREGUE'),p(`Etapa: ${value(d,'approval_title')}`,true,GOLD),p(`Data do envio/entrega: ${datePt(d.delivered_at)}`),p(`Prazo de manifestação: até ${datePt(d.approval_due_at)}`),h('3. MANIFESTAÇÃO DO(A) CONTRATANTE'),p(`${mark(selected(o.acceptance,'accepted'))} Aceito sem ressalvas.`),p(`${mark(selected(o.acceptance,'accepted_with_notes'))} Aceito com ressalvas.`),...(String(o.acceptance_notes??'').trim()?[p(`Ressalvas: ${String(o.acceptance_notes).trim()}`)]:[]),h('4. CONTINUIDADE'),p('O aceite desta etapa não amplia o escopo originalmente contratado e não impede a correção de vícios técnicos.'),h('5. ASSINATURAS'),...signature(profile,d)
 ],profile,code);
 ...
 if(kind==='servico_adicional'){
  ...
  return makeDoc([...title('TERMO DE APROVAÇÃO DE SERVIÇO ADICIONAL','Alteração de escopo • aprovação prévia antes do início'),...identity(d,profile),h('2. ATIVIDADE ADICIONAL'),p(serviceName?`${serviceCode?`(${serviceCode}) `:''}${serviceName}${serviceLevel?` — nível ${serviceLevel}`:''}`:'Atividade adicional não informada.',true,GOLD),h('3. ORIGEM DA NOVA SOLICITAÇÃO'),p(`${mark(reasons.includes('extra_revisions'))} Revisão além das rodadas incluídas.`),p(`${mark(reasons.includes('scope_change'))} Alteração de escopo, premissas, programa, metragem ou layout já aprovado.`),p(`${mark(reasons.includes('level_upgrade'))} Migração para nível de prestação superior ao contratado.`),p(`${mark(reasons.includes('survey'))} Vistoria ou levantamento não incluído no escopo original.`),p(`${mark(reasons.includes('editable_file'))} Arquivo editável ou formato não previsto.`),p(`${mark(reasons.includes('other'))} Outro${String(o.other_reason??'').trim()?`: ${String(o.other_reason).trim()}`:'.'}`),h('4. DESCRIÇÃO DO SERVIÇO ADICIONAL'),p(value(o,'additional_service_description','Não informado')),h('5. IMPACTO COMERCIAL E DE PRAZO'),p(`${mark(pricing==='hour')} Hora técnica`),p(`${mark(pricing==='percentage')} Percentual sobre etapa afetada`),p(`${mark(pricing==='fixed')} Valor fechado aprovado por orçamento específico`),p(`Valor adicional aprovado: ${value(o,'additional_value','Não informado')}`),p(`Forma de pagamento: ${payment==='pix'?'Pix':payment==='bank_transfer'?'Transferência bancária':payment==='card_cash'?'Cartão à vista':payment==='card_installments'?'Cartão parcelado':payment==='cash'?'Dinheiro':payment==='other'?'Outro':'Não informada'}`),p(`Condição de pagamento: ${value(o,'payment_terms','Não informada')}`),...(String(o.new_contract_total??'').trim()?[p(`Novo total contratual informado: ${String(o.new_contract_total).trim()}`)]:[]),p(`Impacto adicional no cronograma: ${value(o,'schedule_impact','Não informado')}`),h('6. APROVAÇÃO'),p(`${mark(approval.includes('approved'))} Aprovo a alteração acima e autorizo o início do serviço adicional nos limites deste termo.`),h('7. ASSINATURAS'),...signature(profile,d)],profile,code);
 }
