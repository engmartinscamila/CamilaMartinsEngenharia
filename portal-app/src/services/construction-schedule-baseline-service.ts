...
export async function listConstructionScheduleBaselines(page: number): Promise<{
  ...
  const response = await supabase.from('construction_schedule_baseline_versions')
    .select('id,schedule_id,baseline_version,baseline_snapshot,approved_at,archived_at', {count: 'exact'})
  ...
  for (const row of response.data ?? []) {
    ...
    if (!snapshot || !Array.isArray(snapshot.activities) || !snapshot.activities.length) {
