...
source=p.read_text()
marker='    function renderizarPendenciasGovernanca() {'
assert source.count(marker)==1
helper=r'''    function conteudoDaRevisao(item) {
...
first=source.index('        box.innerHTML = governanceState.pending.map(item => {')
last=source.index('    function escaparGovernanca(texto) {',first)
old=source[first:last]
assert old.count('Confirmar sem alteração</button>')==1 and old.count('}).join("");')==1
replacement=r'''        box.innerHTML = governanceState.pending.map(item => {
...
assert source.count('function conteudoDaRevisao(item)')==1
assert source.count('Confirmar após conferência</button>')==1
p.write_text(source)
