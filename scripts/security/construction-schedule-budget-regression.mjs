...
// Comentários SQL são documentação, não código executado. Verificar que o cálculo
// não consulta valores comerciais nem campos de honorários como fonte do custo.
const executableSql = rpc.replace(/--[^\r\n]*/g, '');
ok(!/honor[aá]rios?|commercial_total|total_value|quote\.total|contract\.total/i.test(executableSql), 'RPC não usa honorários como custo da obra');
